...
import { pgCode } from "../common/errors";
...
async function dispatch(row: OutboxRow): Promise<void> {
  ...
  switch (row.type) {
    ...
    case "CARD_DELETED":
      ...
      break;
    // Realtime mutation notifications handle these events; the relay only
    // needs to acknowledge them so they leave the durable queue.
    case "CARD_CREATED":
    case "CARD_MOVED":
    case "REALTIME_BOARD_UPDATED":
      break;
    default:
...
async function claimBatch(now: Date): Promise<OutboxRow[]> {
  ...
  // Lock only this batch so concurrent workers do not claim the same rows or
  // accidentally hide an unprocessed backlog behind a large update.
  const claimed = await db.transaction(async (tx) => {
    ...
    return tx
      ...
      .where(inArray(outbox.id, candidates.map((row) => row.id)))
      .returning();
  });
  claimed.sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime() || a.id.localeCompare(b.id));
...
async function processBatch(): Promise<void> {
  ...
  for (const row of rows) {
    try {
      ...
      // Domain rows may be deleted before this asynchronous projection runs.
      // The source transaction is still correct; there is simply nothing left
      // to project, so retrying would only poison the queue.
      if (pgCode(err) === "23503") {
        logger.warn(`relay: discarded stale event ${row.id} (${row.type})`);
        await db.update(outbox).set({ processedAt: new Date(), claimedAt: null }).where(eq(outbox.id, row.id));
        continue;
      }
      const backoffMs = Math.min(60_000 * 2 ** row.attempts, 15 * 60 * 1000);
